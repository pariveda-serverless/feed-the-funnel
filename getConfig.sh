#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`feed-the-funnel-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`feed-the-funnel-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
